...
Un Spark n'expose pas son port 22 : l'accès passe par rebond sur l'hôte.

## Ce qui est cloisonné

Les Sparks sont **non privilégiés**, avec des plages d'identifiants disjointes :
deux Sparks ne partagent aucun UID sur l'hôte. Le quota disque, le plafond réseau
...
- **Un *system container* partage le noyau de l'hôte.** Pour des charges hostiles
  ou réellement multi-locataires, la réponse prévue est un mode machine
  virtuelle, pas un durcissement du mode conteneur.
- **Qui atteint l'hôte atteint le réseau privé.** Le rebond SSH simplifie
  l'accès ; il ne cloisonne pas l'hôte des Sparks.
